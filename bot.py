...
"""
    ...
    keyboard = [
        [InlineKeyboardButton("🎰 Play ₹100", callback_data="slots_100")],
        [InlineKeyboardButton("🎰 Play ₹200", callback_data="slots_200")],
        [InlineKeyboardButton("🎰 Play ₹500", callback_data="slots_500")],
...
async def slots_play(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = update.effective_user.id
    amount = int(query.data.replace("slots_", ""))
    
    user_data = get_user(user_id)
    balance = user_data.get('balance', 0)
    
    if amount < 10:
        await query.edit_message_text("❌ Min bet ₹10!", parse_mode='Markdown')
        return
    if amount > 500:
        await query.edit_message_text("❌ Max bet ₹500!", parse_mode='Markdown')
        return
    if amount > balance:
        await query.edit_message_text(f"❌ Insufficient balance! You have ₹{balance}", parse_mode='Markdown')
        return
    ...
    if win_amount > 0:
        new_balance = balance - amount + win_amount
        profit = win_amount - amount
        update_user(user_id, {"balance": new_balance})
        result += f"💰 **Bet:** ₹{amount}\n"
        result += f"🏆 **Win:** ₹{win_amount}\n"
        result += f"📈 **Profit:** +₹{profit}\n"
        result += f"💎 **New Balance:** ₹{new_balance}\n\n"
        result += win_text
    else:
        ...
        result += f"💰 **Bet:** ₹{amount}\n"
        result += f"🏆 **Win:** ₹0\n"
        result += f"📉 **Loss:** -₹{amount}\n"
        result += f"💎 **New Balance:** ₹{new_balance}\n\n"
        result += win_text
    
    keyboard = [
        [InlineKeyboardButton("🔁 Play Again", callback_data=f"slots_{amount}")],
        [InlineKeyboardButton("⬅️ Back", callback_data="back")]
    ]
...
async def slots_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Enter valid amount!", parse_mode='Markdown')
        return
    ...
    if amount < 10:
        await update.message.reply_text("❌ Min bet ₹10!", parse_mode='Markdown')
        return
    if amount > 500:
        await update.message.reply_text("❌ Max bet ₹500!", parse_mode='Markdown')
        return
    if amount > balance:
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        await update.message.reply_text("🚫 **Not admin!**", parse_mode='Markdown')
        return
